...
class ScreenProductNew extends StatefulWidget {
  ...
  ScreenProductNew(this.Data,this.DataSelected);



  @override
  State<ScreenProductNew> createState() => _ScreenCustomerNewState();
}

class _ScreenCustomerNewState extends State<ScreenProductNew> {




  List<ResultSearchProduct> Customer = <ResultSearchProduct>[];
  ...
  void Run(){
    for (var element in Customer) {
      if(DataSelectedMain.contains(element.shka))
        {
    ...
    return Scaffold(
                          ...
                          child: TextField(
                            ...
                            onChanged: (val) async{
                              if(val.isNotEmpty)
                              {
                                ...
                                Customer.clear();
                             var   CustomerData = await ApiService.GetProductNew(pr, Baseurl, UserName, Password,widget.Data,val.toString());
                                Customer=CustomerData.result;
                                Customer.sort((a, b) => a.productName.compareTo(b.productName));
                                Run();
                                ...
                                return   InkWell(
                                  onTap: () async{
                                    for (int i=0;i<DataSelectedMain.length;i++) {
                                      if(DataSelectedMain[i]==Customer[item].shka)
                                        {
      ...
      Padding(
        ...
        child: FloatingActionButton(
          ...
          onPressed: (){
            List<ResultSearchProduct> TempCustomer=[];
            for (var element in Customer) {
