...
import '../NotSaleProduct/ItemNoSaleCustomer.dart';
import '../PathComponentOneChoose/MainItemFilterPath.dart';

import '../ProductGroupComponent/ItemGridProduct.dart';
import '../ProductGroupComponent/MainItemFilterProductGroup.dart';
...
import '../ScreenProductNew.dart';
import '../VisitorComponent/MainItemFilterVisitor.dart';
import 'Boxinfonotsupportproduct.dart';


class ScreenNoSupportCustomer extends StatefulWidget {
  @override
  State<ScreenNoSupportCustomer> createState() => _ScreenNoSupportProductState();
}



class _ScreenNoSupportProductState extends State<ScreenNoSupportCustomer> {
  ...
  List<ResultSearchProduct> Products=[];
  List<ResultGroupProduct> GroupProduct=[];
                                        ...
                                        Checkbox(
                                          ...
                                          onChanged: (bool? value) {
                                            state(() {
                                              ...
                                              if(Flag=='7')
                                              {
                                                IsAllGroupPruduct=!IsAllGroupPruduct;
                                              }
                                    ...
                                    child: ListView.builder(
                                      ...
                                      itemBuilder: (ctx,item){
                                        return
                                          ...
                                          Container(
                                            child: InkWell(
                                              ...
                                              onTap: ()
                                              {
                                                ...
                                                if(Flag=='7')
                                                {
                                                  if(IsAllGroupPruduct==false)
                                                  {
                                                    state(() {
                                                      GroupProduct[item].IsCheck= !GroupProduct[item].IsCheck;
                                                    });
                                                  }
                                                  // Navigator.pop(context);
                                                  ...
                                                  Checkbox(
                                                    ...
                                                    onChanged: (bool? value) {
                                                      ...
                                                      if(Flag=='7')
                                                      {
                                                        ...
                                                        {
                                                          state(() {
                                                            GroupProduct[item].IsCheck=value!;
                                                          });
                                                        }
                                                        // Navigator.pop(context);
                                          ...
                                          txtsearch.text.isNotEmpty && Flag=='7'&&  GroupProduct[item].groupName.contains(txtsearch.text.toString())?
                                          Container(
                                            child: InkWell(
                                              highlightColor: Colors.transparent,
                                              onTap: ()
  ...
  Future Run(String Flag) async{
    ...
    if(Flag=='7')
    {
      ...
      var data=await ApiService.GetGroupProduct(PRo,base!,UserName!,Password!);
      PRo.hide();
  ...
  List<ResultNotSaleCostomer>  mainItems=[];

  Future GetData()async{
    List<int> visid=[];
    List<int> Pathh=[];
    List<int> CustGr=[];
    List<int> GroupProducttemp=[];
    List<int> producttemp=[];
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var base =prefs.getString('Baseurl');
    var UserName =prefs.getString('UserName');
    var Password =prefs.getString('Password');
    ...
    if(IsAllGroupPruduct)
    {
      GroupProduct.forEach((element) {
        GroupProducttemp.add(element.groupRdf);
      });
    }else if(Path.isNotEmpty)
    {
      GroupProduct.forEach((element) {
    ...
    var Data= await ApiService.GetCustomerNotSale(pr,base!,UserName!,Password!,Ta_Data,Az_Data,GroupProducttemp,Pathh,producttemp,CustGr,visid);
    pr.hide();
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      MainItemFilterProductGroup(
                          IsAllGroupPruduct,GroupProduct.where((element) => element.IsCheck==true).toList(),(){
                           if(GroupProduct.isNotEmpty)
                             {
                               ShowModall_('7');
                             }else{
                             Run('7');
                      ...
                      InkWell(
                        onTap: ()async{
                          ...
                          List<int> datamainCusGroupSelected=[];
                          if(GroupProduct.isEmpty)
                            {
                              ApiService.ShowSnackbar('گروه محصول را انتخاب کنید');
                              return;
                            }
                          ...
                          if(IsAllGroupPruduct==true)
                            {
                            ...
                            }else
                              {
                                GroupProduct.forEach((element) {
                                  if(element.IsCheck)
                                    {
                                      datamainCusGroup.add(element.groupRdf);
                                    }
                                });
                              }

                          if(datamainCusGroup.isEmpty&&IsAllGroupPruduct==false)
                          {
                            ApiService.ShowSnackbar('گروه محصول را انتخاب کنید');
                            return;
                          }
                          ...
                          Products.forEach((element) {
                            if(element.IsCheck)
                              {
                                datamainCusGroupSelected.add(element.shka);
                              }
                          });

                          var datatemp   = await  Navigator.of(context).push(
                              MaterialPageRoute(builder: (context) => ScreenProductNew(datamainCusGroup,datamainCusGroupSelected)));
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  Expanded(child: ColumItem('کالا')),
                                ],
                              ),
                            IsAllProduct==true|| Products.length>0?
                              Container(
                                child: Directionality(
                                  textDirection: TextDirection.rtl,
                                  child: GridView.builder(
                                    itemCount: IsAllProduct==true?1:
                                    Products.isEmpty?0:
                                    Products.length,
                                    shrinkWrap: true,
                      ...
                      // MainItemFilterCustomer(
                      //     IsAllCustomer,Customers.where((element) => element.IsCheck==true).toList(),(){
                      //   Run('6');
                      // },(){
                      //   setState(() {
                      //   });
        ...
        RefreshIndicator(
          ...
          child: Container(
            ...
            child: ListView.builder(
                ...
                // itemCount: 3,
                itemBuilder: (ctx,item){
                  return ItemNoSaleCustomer(mainItems[item]);
                  // return  InkWell(
                  //     onTap: (){
                  //
                  //     },
